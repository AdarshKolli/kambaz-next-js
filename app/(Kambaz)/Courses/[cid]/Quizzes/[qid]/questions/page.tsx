...
export default function QuestionsEditor() {
  ...
  const handleCancelEdit = () => {
    ...
    fetchQuestions(); // Refresh to discard any changes
  };
  ...
  const handleSaveQuestion = async () => {
    ...
    if (editingQuestion.type === "fill-in-blank") {
      // Validate each blank has at least one answer
    ...
    try {
      if (editingQuestion._id) {
        ...
        const saved = await quizzesClient.createQuestion(qid as string, editingQuestion);
  ...
  const handleSaveAndPublish = async () => {
    try {
      ...
      await quizzesClient.updateQuiz(qid as string, { ...quizData, published: true });
  ...
  return (
    <div id="wd-questions-editor" className="p-3">
      ...
      <ListGroup className="mb-3">
        {questions.map((question, index) => (
          <div key={question._id}>
            ...
            {/* Show editor right after this question if it's being edited */}
      ...
      {/* Show editor at the bottom for new questions (no _id) */}
